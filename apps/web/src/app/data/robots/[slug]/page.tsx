...
export async function generateMetadata({ params }: IPageProps): Promise<Metadata> {
  ...
  const specs: string[] = []
  if (robot.dof !== null) specs.push(`${robot.dof} DOF`)
  if (robot.weight_kg !== null) specs.push(`${robot.weight_kg} kg`)
  if (robot.price_usd !== null) specs.push(`$${robot.price_usd.toLocaleString()}`)
  const specStr = specs.length > 0 ? ` ${specs.join(", ")}.` : ""
  const sensorStr = robot.sensors.length > 0 ? ` Sensors: ${robot.sensors.slice(0, 3).join(", ")}.` : ""
  const desc = `${robot.description}${specStr}${sensorStr}`.slice(0, 300)
  return {
    title: `${robot.name} by ${robot.manufacturer} | Festivus`,
    description: desc,
    openGraph: {
      title: `${robot.name} by ${robot.manufacturer}`,
      description: desc,
      ...(robot.image_url ? { images: [{ url: robot.image_url }] } : {}),
    },
  }
}
...
export default async function RobotProfilePage({ params }: IPageProps) {
  ...
  const jsonLd = {
    ...
    manufacturer: { "@type": "Organization", name: robot.manufacturer },
    category: `Physical AI / ${robot.category}`,
    ...(robot.image_url ? { image: robot.image_url } : {}),
    ...(robot.product_page_url ? { url: robot.product_page_url } : {}),
    ...(robot.weight_kg !== null ? { weight: { "@type": "QuantitativeValue", value: robot.weight_kg, unitCode: "KGM" } } : {}),
    ...(robot.price_usd !== null ? { offers: { "@type": "Offer", price: robot.price_usd, priceCurrency: "USD" } } : {}),
    additionalProperty: [
      ...(robot.dof !== null ? [{ "@type": "PropertyValue", name: "Degrees of Freedom", value: robot.dof }] : []),
      { "@type": "PropertyValue", name: "Form Factor", value: robot.form_factor },
      { "@type": "PropertyValue", name: "Deploy Readiness", value: robot.deploy_readiness },
      ...(robot.actuators ? [{ "@type": "PropertyValue", name: "Actuators", value: robot.actuators }] : []),
      ...(robot.sensors.length > 0 ? [{ "@type": "PropertyValue", name: "Sensors", value: robot.sensors.join(", ") }] : []),
    ],
  }
