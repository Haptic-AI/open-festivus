...
describe("sitemap", () => {
  ...
  it("includes static routes plus a slug entry for every entity type", async () => {
    ...
    // Policy emitted only under /data/policies (canonical); /policies/[slug] uses canonical tag instead
    expect(urls).toContain("https://festivus.hapticlabs.ai/data/policies/openvla-7b")
    expect(urls).not.toContain("https://festivus.hapticlabs.ai/policies/openvla-7b")
  })
  ...
  it("emits policies only under /data/policies (canonical URL)", async () => {
    globalThis.fetch = endpointFetch({
