...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  // High-value curated entities get top priority
  pushSlugs(robots, "/data/robots", 0.8)
  pushSlugs(simulations, "/data/simulations", 0.7)
  pushSlugs(environments, "/data/environments", 0.7)
  pushSlugs(tasks, "/data/tasks", 0.6)
  pushSlugs(deployNotes, "/data/deploys", 0.5)

  // Bulk entities stay in sitemap at lower priority
  pushSlugs(datasets, "/data/datasets", 0.4)
  pushSlugs(benchmarks, "/data/benchmarks", 0.4)
  pushSlugs(papers, "/data/papers", 0.4)

  // Policies: only emit under /data/policies/ (canonical).
  // /policies/[slug] gets a canonical tag pointing here instead of
  // duplicating every slug in the sitemap.
  pushSlugs(policies, "/data/policies", 0.5)
